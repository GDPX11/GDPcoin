...
const string strMessageMagic = "GDPcoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    {
		nSubsidy = 24061886 * COIN;
    }
	else if(pindexBest->nHeight < 2001)
    {
		nSubsidy = 8399.85 * COIN;
    }
	else if(pindexBest->nHeight < 4001)
    {
		nSubsidy = 4590.6885 * COIN;
    }
	else if(pindexBest->nHeight < 6001)
    {
		nSubsidy = 2450.766 * COIN;
    }
	else if(pindexBest->nHeight < 8001)
    {
		nSubsidy = 1817.9795 * COIN;
    }
	else if(pindexBest->nHeight < 10001)
    {
		nSubsidy = 1368.6805 * COIN;
    }	
	else if(pindexBest->nHeight < 12001)
    {
		nSubsidy = 1267.8805 * COIN;
    }	
	else if(pindexBest->nHeight < 14001)
    {
		nSubsidy = 1121.427 * COIN;
    }	
	else if(pindexBest->nHeight < 16001)
    {
		nSubsidy = 1059.003 * COIN;
    }	
	else if(pindexBest->nHeight < 18001)
    {
		nSubsidy = 1035.9775 * COIN;
    }	
	else if(pindexBest->nHeight < 20001)
    {
		nSubsidy = 935.3255 * COIN;
    }
	else if(pindexBest->nHeight < 22001)
    {
		nSubsidy = 912.548 * COIN;
    }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "GDPcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "GDPcoin Countries of the former list of 11 GDP-based.";
        CTransaction txNew;
        txNew.nTime = 1402070400;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1402070400;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1767682;
		if(fTestNet)
        {
            block.nNonce   = 0;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x8151baa33b78bcec5ce9a8206fcb56ed7ba82af211a6b58fd6ca9bd3386da1c7"));
        block.print();
